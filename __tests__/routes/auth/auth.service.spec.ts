...
vi.mock("@/common/providers/auth-constant.provider");
...
describe("AuthService", () => {
  ...
  describe("로그인", () => {
    it("성공", async () => {
      authService.signature = vi.fn().mockReturnValue(TOKEN_STUB);

      const result = await authService.signin(TOKEN_USER_STUB);

      expect(authService.signature).toHaveBeenCalledTimes(2);
      expect(authService.signature).toHaveReturnedWith(TOKEN_STUB);
      expect(result).toEqual([TOKEN_STUB, TOKEN_STUB]);
    });
  });

  describe("refresh token 유효성 검사", () => {
    let jwtServiceVerifySpy: TestSpyInstance;

    beforeEach(() => {
      jwtServiceVerifySpy = vi.spyOn(jwtService, "verify");
    });

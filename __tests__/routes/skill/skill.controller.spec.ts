...
vi.mock("@/routes/skill/skill.service");
...
describe("SkillController", () => {
  ...
  describe("스킬 생성", () => {
    it("성공", async () => {
      const serviceCreateSpy = vi.spyOn(service, "create").mockResolvedValueOnce(SKILL_STUB);
      ...
      expect(serviceCreateSpy).toHaveBeenCalledWith(CREATE_SKILL_STUB);
    });
  });

  describe("스킬 수정", () => {
    it("성공", async () => {
      const serviceUpdateSpy = vi.spyOn(service, "update").mockResolvedValueOnce(SKILL_STUB);
      ...
      expect(serviceUpdateSpy).toHaveBeenCalledWith(SKILL_STUB._id, CREATE_SKILL_STUB);
    });
  });
  ...
  describe("스킬 삭제", () => {
    it("성공", async () => {
      ...
      expect(serviceDeleteSpy).toHaveBeenCalledWith(SKILL_STUB._id);
    });
  });

...
vi.mock("@/routes/tag/tag.repository");
...
describe("TagService", () => {
  ...
  describe("태그 조회", () => {
    it("태그 전체 조회", async () => {
      const repositoryGetAllSpy: TestSpyInstance = vi.spyOn(repository, "getAllToA");
    ...
    it("태그 이름으로 조회", async () => {
      const repositoryGetOneSpy: TestSpyInstance = vi.spyOn(repository, "getOne");
  ...
  describe("태그에 게시글 추가", () => {
    it("성공", async () => {
      const repositoryFindOrCreateSpy: TestSpyInstance = vi.spyOn(repository, "findOrCreate");
      const repositoryAddPostIdInTagsSpy: TestSpyInstance = vi.spyOn(
        repository,
        "addPostIdInTags",
      );
  ...
  describe("태그에 게시글 삭제", () => {
    it("태그 이름 배열을 받아 해당 이름과 일치하는 태그를 찾아 게시글 아이디를 삭제한다", async () => {
      const repositoryPullPostIdInTagsSpy: TestSpyInstance = vi.spyOn(
        repository,
        "pullPostIdInTags",
      );
      const repositoryGetAllSpy: TestSpyInstance = vi.spyOn(repository, "getAll");
    ...
    it("게시글 아이디를 받아 해당 게시글을 가지고 있는 태그를 찾아 게시글 아이디를 삭제한다", async () => {
      const repositoryPullPostIdByPostIdSpy: TestSpyInstance = vi.spyOn(
        repository,
        "pullPostIdByPostId",
      );
